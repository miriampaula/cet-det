...
Embeddings are still extracted for ALL rows (including DRYAD) so the
LODO eval script can use them without modification.

Usage:
  cd /data2/mromaniuc/cet-det/student_teacher
  mkdir -p logs
  nohup python 9-no-dryad.py > logs/v09_no_dryad.log 2>&1 &
  echo $! > logs/v09_no_dryad.pid
  tail -f logs/v09_no_dryad.log
"""
...
VARIANTS = {
    'v09_best_combo_no_dryad': dict(
        ...
        note='v09 retrained without DRYAD — honest LODO check',
    ),
}
...
# ── Hold out DRYAD entirely from student training ───────────────────────────
# Must happen AFTER encoding so dataset_idx covers all datasets (needed at
# inference time when we embed the full corpus including DRYAD).
HOLDOUT_DATASET = 'DRYAD'
...
def run_variant(name, cfg):
    ...
    # train/val/test all drawn from meta_train_only (no DRYAD)
    ...
    # ── Extract embeddings for ALL rows including DRYAD ─────────────────────
...
summary_df.to_csv(RUNS_DIR / 'variants_summary_no_dryad.csv', index=False)
print("\n" + "═"*72)
print("  DONE")
print("═"*72)
print(summary_df.to_string(index=False))
print(f"\nSummary → {RUNS_DIR / 'variants_summary_no_dryad.csv'}")
