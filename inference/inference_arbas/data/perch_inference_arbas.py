...
perch_inference.py
------------------
Runs Perch V2 inference on a set of WAV files using multiple workers.
...
Input can be specified EITHER by:
  --wav_folder  <folder>     scan a folder recursively for .wav files, OR
  --file_list   <file.txt>   read one WAV path per line (e.g. from
                             select_arbas_wavs.py)

Usage
-----
python perch_inference_arbas.py ^
    --file_list "C:/Users/surra/inference-arbas/selected_files/arbas_wav_paths.txt" ^
    --save_dir "C:/Users/surra/inference-arbas/embeddings" ^
    --window_size 5.0 ^
    --batch_size 32 ^
    --workers 1 ^
    --checkpoint_every 5
...
def parse_args():
    ...
    src.add_argument("--wav_folder", help="Folder to scan recursively for .wav files")
    src.add_argument("--file_list",  help="Text file with one WAV path per line")
...
def make_windows(audio: np.ndarray, sr: int, window_size_s: float):
    ...
    # Check if there is a leftover tail worth keeping (at least 0.1s of audio)
    tail_start = n_windows * samples_per_window
    tail_length = len(audio) - tail_start
    has_tail = tail_length >= int(0.1 * sr)

    for i in range(n_windows):
        start = i * samples_per_window
        yield i, i * window_size_s, audio[start : start + samples_per_window]

    # Yield tail window zero-padded to full window size
    if has_tail:
        tail = audio[tail_start:]
        padded = np.zeros(samples_per_window, dtype=np.float32)
        padded[:len(tail)] = tail
        yield n_windows, n_windows * window_size_s, padded
...
def worker_fn(worker_id: int, wav_paths: list[str], save_dir: str,
    ...
    def save_checkpoint(emb_list, lgt_list, meta_list, log_list, idx):
        ...
        # Append to shared processing_log.csv with cross-platform lock
...
def main():
    ...
    if args.wav_folder:
        log.info(f"  wav_folder       : {args.wav_folder}")
    else:
        log.info(f"  file_list        : {args.file_list}")
    ...
    if args.wav_folder:
        ...
        all_wav_files = [str(w) for w in all_wav_files]
    else:
        list_path = Path(args.file_list)
        if not list_path.exists():
            raise FileNotFoundError(f"file_list does not exist: {list_path}")
        with open(list_path, "r", encoding="utf-8") as f:
            all_wav_files = [line.strip() for line in f if line.strip()]
        if not all_wav_files:
            raise ValueError(f"file_list is empty: {list_path}")
    ...
    remaining = [w for w in all_wav_files if w not in already_done]
